...
n_objs = np.asarray([5,7,9])  # number of objectives
        ...
        for problem_name in problem_names:
            ...
            for run in range(total_run):
                ...
                for i in range(L):
                    ...
                    #response = response.T
                    int_rvea.set_interaction_type('Preferred solutions')
                    pref_int_rvea, _ = int_rvea.start()
                ...
                for i in range(D):
                    ...
                    response = gp.preferredSolutions4decision(base)
                    #response = response.T
                    data_row["preferred_solutions"] = [response]
                    # run algorithms with the new reference point
                    pref_int_rvea.response = response
