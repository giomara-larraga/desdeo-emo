...
n_objs = np.asarray([7,9])  # number of objectives
...
algorithms = ["iRVEA", "RNSGAIII"]  # algorithms to be compared
...
for gen in num_gen_per_iter:
    for n_obj, n_var in zip(n_objs, n_vars):
        for problem_name in problem_names:
            ...
            for run in range(total_run):
                ...
                #RNSGAIII
                int_rnsga = RNSGAIII(problem, 50, np.atleast_2d(UPEMO(response)), n_iterations=10, n_gen_per_iter=gen) 
                pref_int_nsga,_ = int_rnsga.iterate()
                ...
                (
                    ...
                    int_rvea.population.objectives, int_rnsga.population.objectives
                )
                ...
                for i in range(L):
                    ...
                    #response = response.T
                    int_rvea.set_interaction_type('Preferred ranges')
                    pref_int_rvea, _ = int_rvea.start()
                    data_row["preferred_ranges"] = [response]
                    # run algorithms with the new reference point
                
                    #Interactive RVEA
                    pref_int_rvea.response = response
                    previous_RVEA_FEs = int_rvea._function_evaluation_count
                    
                    pref_int_rvea,_ = int_rvea.iterate(pref_int_rvea)
                    int_rnsga = RNSGAIII(problem, 50, np.atleast_2d(UPEMO(response.T)), n_iterations=10, n_gen_per_iter=gen) 
                    pref_int_rnsga,_ = int_rnsga.iterate()
                    previous_NSGA_FEs = int_rnsga._function_evaluation_count
                    peritr_RVEA_FEs = (
                        int_rvea._function_evaluation_count - previous_RVEA_FEs
                    )
                    peritr_NSGA_FEs = (
                        int_rnsga._function_evaluation_count - previous_NSGA_FEs
                    )
                    ...
                    nsga_transformer = Normalizer().fit(int_rnsga.population.objectives)
                    ...
                    rm_nsga_learning = np.append(rm_nsga_learning, rigd_insga)
                ...
                for i in range(D):
                    ...
                    rm_nsga_decision = np.append(rm_nsga_decision, rigd_insga)
    ...
    #data.to_csv(f"ranges{run+1}.csv", index=False)
