        ...
        for problem_name in problem_names:
            ...
            for run in range(total_run):
                ...
                for i in range(L):
                    ...
                    int_nsga.set_interaction_type('Preferred ranges')
                    int_nums.set_interaction_type('Preferred ranges')
                    int_rnsga.set_interaction_type('Preferred ranges')
                    ...
                    norm_nums = nums_transformer.transform(
                        int_nums.population.objectives
                    )

                    rnsga_transformer = Normalizer().fit(int_rnsga.population.objectives)
                    norm_rnsga = rnsga_transformer.transform(
                        int_rnsga.population.objectives
                    )
                    # R-metric calls for R_IGD and R_HV
                 
                    rigd_nsga, rhv_nsga = rmetric.calc(norm_nsga, others=np.vstack((norm_nums,norm_rnsga)))
                    rigd_nums, rhv_nums = rmetric.calc(norm_nums, others=np.vstack((norm_nsga,norm_rnsga)))
                    rigd_rnsga, rhv_rnsga = rmetric.calc(norm_rnsga, others=np.vstack((norm_nsga,norm_nums)))
                    ...
                    rm_nsga_learning = np.append(rm_nsga_learning, rigd_nsga)
                    rm_nums_learning = np.append(rm_nums_learning, rigd_nums)
                    rm_rnsga_learning = np.append(rm_rnsga_learning, rigd_rnsga)
                ...
                median_rm_nums_learning = np.append(median_rm_nums_learning, np.sum(rm_nums_learning))
                median_ns_rnsga_learning = np.append(median_ns_rnsga_learning, np.sum(ns_rnsga_learning))
                median_rm_rnsga_learning = np.append(median_rm_rnsga_learning, np.sum(rm_rnsga_learning))
                ...
                for i in range(D):
                    ...
                    #] = [rigd_rnsga, rhv_rnsga]
                    #data = data.append(data_row, ignore_index=1)
                    rm_nsga_decision = np.append(rm_nsga_decision, rigd_nsga)
                    rm_nums_decision = np.append(rm_nums_decision, rigd_nums)
                    rm_rnsga_decision = np.append(rm_rnsga_decision, rigd_rnsga)
                ...
                median_rm_nums_decision = np.append(median_rm_nums_decision, np.sum(rm_nums_decision))
                median_ns_rnsga_decision = np.append(median_ns_rnsga_decision, np.sum(ns_rnsga_decision))
                median_rm_rnsga_decision = np.append(median_rm_rnsga_decision, np.sum(rm_rnsga_decision))
            ...
            print("RM")
            print (f"{np.mean(median_rm_nsga_learning):.4f} & {np.std(median_rm_nsga_learning):.4f} & {np.mean(median_rm_nums_learning):.4f} & {np.std(median_rm_nums_learning):.4f} & {np.mean(median_rm_rnsga_learning):.4f} & {np.std(median_rm_rnsga_learning):.4f}")
            ...
            print("RM")
            print (f"{np.mean(median_rm_nsga_decision):.4f} & {np.std(median_rm_nsga_decision):.4f} & {np.mean(median_rm_nums_decision):.4f} & {np.std(median_rm_nums_decision):.4f} & {np.mean(median_rm_rnsga_decision):.4f} & {np.std(median_rm_rnsga_decision):.4f}")
