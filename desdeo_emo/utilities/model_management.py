...
def remove_duplicate(X: np.ndarray, archive_x: np.ndarray):
    ...
    repeated_all_variables = np.delete(
        all_variables_indicies, unique_variables_indicies
    )
...
def ikrvea_mm(
    ...
    nd = remove_duplicate(
        individuals, problem.archive.drop(problem.objective_names, axis=1).to_numpy()
    ...
    asf_solutions = SimpleASF([1] * problem.n_of_objectives).__call__(
        non_duplicate_obj, reference_point
    )
...
def krvea_mm(
    ...
    nd = remove_duplicate(
        ...
        problem.archive.drop(problem.objective_names, axis=1).to_numpy(),
    )  # removing duplicate solutions
    if len(nd) == 0:
        return problem
    else:
        non_duplicate_dv = population.individuals[nd]
        non_duplicate_obj = population.objectives[nd]
        non_duplicate_unc = population.uncertainity[nd]
