...
class APD_Select(SelectionBase):
    ...
    def do(self, pop: Population) -> List[int]:
        ...
        for i in range(0, len(self.vectors.values)):
            ...
            if len(sub_population_fitness > 0):
                self.active_adaptive_RVs = np.append(self.active_adaptive_RVs, i)
                # APD Calculation
    ...
    def cluster_active_RVs(num_individuals: int):
